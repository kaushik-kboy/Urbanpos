...
class DynamicValidationService
{
    ...
    private array $autoGeneratedDocNumbers = [
        'bill_number',
        'invoice_number',
        'po_number',
        'return_number',
    ...
    public function applyTo(string $moduleKey, array &$rules, array &$messages, mixed $ignoreId = null): void
    {
        try {
            ...
            foreach ($configs as $fieldName => $config) {
                ...
                if ($config->is_unique) {
                    // System-generated sequence numbers (like bill_number, invoice_number, grn_number, etc.)
                    // are generated server-side. On create ($ignoreId is empty), multiple users opening the
                    // create form simultaneously might submit the same preview number. We must not fail validation
                    // on create because store() dynamically allocates a fresh unique sequence.
                    if (empty($ignoreId) && in_array($targetField, $this->autoGeneratedDocNumbers, true)) {
